...
case "$TARGET" in
  *-windows-*)
    ...
    # CARGO_ENCODED_RUSTFLAGS uses \x1f (unit separator) between flags.
    CARGO_ENCODED_RUSTFLAGS=$(cargo rustflags --target "$TARGET" -1 | tr '\n' $'\x1f' | sed $'s/\x1f$//')
...
case "$TARGET" in
  *-windows-*)
    ...
    echo "--- PE imports ---"
    llvm-readobj --coff-imports native/next-swc.*.node | head -50 || true
    ;;
  *)
